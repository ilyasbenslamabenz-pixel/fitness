...
async function main() {
  ...
  /* icônes en PNG uniquement : Safari iOS ignore le JPEG (manifeste et apple-touch-icon)
     et affiche alors des initiales générées */
  for (const n of [180, 192, 512]) await sharp(icon).resize(n, n, { kernel: "lanczos3" }).png({ compressionLevel: 9 }).toFile(path.join(ROOT, "icon-" + n + ".png"));
